...
sub check_finished_samples {
    my $query_running_sample = "SELECT sampleID,analysisID FROM sampleInfo WHERE currentStatus = '4';";
...
sub update_qualMetrics {
    my $sampleID = shift;
    my $analysisID = shift;
    my $query = "SELECT jobName FROM hpfJobStatus WHERE jobName IN ($SQL_JOBLST) AND exitcode = '0' AND sampleID = '$sampleID' AND analysisID = '$analysisID' ";
    ...
    if ($sthQUF->rows() != 0) {
        ...
        my $cmd = "$SSHDATA $HPF_RUNNING_FOLDER $sampleID-$analysisID $joblst\"";
        my @updates = `$cmd`;
        if ($? != 0) {
        ...
        &run_update(@updates);
        $query = "UPDATE sampleInfo SET currentStatus = '" . &check_qual($sampleID, $analysisID, @updates) . "' WHERE sampleID = '$sampleID' AND analysisID = '$analysisID'";
        print $query,"\n";
        $sthQUF = $dbh->prepare($query);
        $sthQUF->execute();
    }
    else {
        my $msg = "No successful job generate sql file for sampleID $sampleID analysisID $analysisID ? it is impossible!!!!\n";
        print $STDERR $msg;
        email_error($msg);
        return 2;
    }
        ...
        foreach (split(/,/)) {
            ...
            if (exists $FILTERS{$tmmp[0]}) {
                if (not eval($tmmp[1] . $FILTERS{$tmmp[0]})) {
                    $msg .= $tmmp[0] . " = ". $tmmp[1] . " of sampleID $sampleID analysisID $analysisID failed to pass the filter: " . $FILTERS{$tmmp[0]} . "\n";
                }
            }
        }
    }
