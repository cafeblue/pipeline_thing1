...
my $copy = $dbhC->prepare("INSERT INTO sampleSheet SELECT * from clinicalA.sampleSheet where TIMESTAMPADD(MINUTE,10,time)>=CURRENT_TIMESTAMP") or die "Can't query database\n";
$copy->execute();
...
$copy = $dbhC->prepare("INSERT INTO sampleInfo SELECT * from clinicalA.sampleInfo where TIMESTAMPADD(MINUTE,10,time)>=CURRENT_TIMESTAMP and genePanelVer = 'cancer.gp19'") or die "Can't query database for running samples: ". $dbhC->errstr() . "\n";
$copy->execute();
