...
sub main {
    ...
    while(my $vcf_data = $vcf->next_data_hash()) {
        ...
        my ($tumour_ref_count, $tumour_alt_count) = split(/,/, $vcf_data->{'gtypes'}->{'TUMOR'}->{'AD'});
        my ($normal_ref_count, $normal_alt_count) = split(/,/, $vcf_data->{'gtypes'}->{'NORMAL'}->{'AD'});
        my $variant_length;
        ...
        print {$ofh} join("\t",
            ...
            $vcf_data->{'INFO'}->{'NLOD'},
            $vcf_data->{'INFO'}->{'TLOD'},
            $vcf_data->{'gtypes'}->{'NORMAL'}->{'GT'},
            $normal_ref_count,
            $normal_alt_count,
            $normal_ref_count + $normal_alt_count,
            $vcf_data->{'gtypes'}->{'NORMAL'}->{'AF'},
            $vcf_data->{'gtypes'}->{'TUMOR'}->{'GT'},
            $tumour_ref_count,
            $tumour_alt_count,
            $tumour_ref_count + $tumour_alt_count,
            $vcf_data->{'gtypes'}->{'TUMOR'}->{'AF'},
            $variant_length,
            $mutation_type
            ), "\n";
        }
...
mutect2annovar.pl --vcf file.vcf --output file.annovar --filter false --header false
