...
if ($allerr ne '') {
    print STDERR $allerr;
    Common::email_error("Error on chksum for fastq", $allerr, "NA", "NA", "NA", $config->{'EMAIL_WARNINGS'} );
}
...
sub checksum_fastq {
    ...
    my $total_sampleNum = `ls $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/*_L001_R1_001.fastq.gz|wc -l`;
    chomp($total_sampleNum);
    $total_sampleNum--;
    ####### rename the fastq files 
    for my $id (1..$total_sampleNum) {
        my @files = `ls $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/*_S$id\_L00?_R1_001.fastq.gz `;
        chomp(@files);
        foreach my $loca (@files) {
            ...
            if ($filename =~ /(.+?)_S$id\_L00(\d)_R1_001.fastq.gz/) {
                ...
                my $order = $2;
                my $cmd = "mv $loca $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/$sampleID\_$flowcellID\_R1_$order\.fastq.gz";
                print $cmd,"\n";
                `$cmd`;
                $loca =~ s/_R1_001\.fastq\.gz/_R2_001\.fastq\.gz/;
                $cmd = "mv $loca $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/$sampleID\_$flowcellID\_R2_$order\.fastq.gz";
                print $cmd,"\n";
                `$cmd`;
            }
            else {
                my $msg = "file $loca in error format, rename file aborted\n";
                Common::email_error("Error on chksum for fastq", $msg, $machine, "NA", $flowcellID, $config->{'EMAIL_WARNINGS'});
                Common::cronControlPanel($dbh, 'chksum_fastq', "STOP");
                die "$msg\n";
            }
        }
    ...
    if ($sthQNS->rows() != 0) {  #no samples are being currently sequenced
        ...
        while (my @data_ref = $sthQNS->fetchrow_array) {
            if (not exists $sampleID_lst{$data_ref[0]}) {
                my $msg .= "SampleID: " . $data_ref[0] . " can't be found under the folder of $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID\n ";
            }
            else {
        ...
        if (scalar(keys %sampleID_lst) != 0) {
            ...
            Common::email_error("Error on chksum for fastq", $missed_samples, $machine, "NA", $flowcellID, $config->{'EMAIL_WARNINGS'});
            Common::cronControlPanel($dbh, 'chksum_fastq', "STOP");
            die $missed_samples;
        }
        if ($msg ne "") {
    ...
    my @gz_files = `ls $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/*_$flowcellID\_*.fastq.gz`;
    my @all_fastq_names = ();
    my @commands;
    chomp(@gz_files);
    foreach my $source ( @gz_files ) {
        ...
        my $cmd = "cd $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/ ; sha256sum $filename > $filename.sha256sum";
        push @commands, $cmd;
    }
    ...
    `$SSHCMD "mkdir $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID"`;
    print "$SSHCMD \"mkdir $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID\"\n";
    my $msg = "";
    if ($? != 0 ) {
        $msg .= "error sshmsg: create directory $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID on hpf failed with error code: $?\n";
    }
    foreach my $sampleID ( keys %sampleIDs ) {
        `$SSHCMD "mkdir $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID/Sample_$sampleID"`;
        print "$SSHCMD \"mkdir $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID/Sample_$sampleID\"\n";
        if ( $? != 0 ) {
            $msg .= "error ssh msg: create directory $config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID/Sample_$sampleID for $machine, $flowcellID on hpf failed with error code: $?\n";
        }
        `$RSYNCCMD $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/$sampleID\_* wei.wang\@data1.ccm.sickkids.ca:$config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID/Sample_$sampleID `;
        print "$RSYNCCMD $config->{'FASTQ_FOLDER'}/TESTINGTESTING$machine\_$flowcellID/$sampleID\_* wei.wang\@data1.ccm.sickkids.ca:$config->{'FASTQ_HPF'}/TESTINGTESTING$flowcellID/Sample_$sampleID \n";
        if ( $? != 0 ) {
